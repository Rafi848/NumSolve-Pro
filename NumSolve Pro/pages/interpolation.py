...
.back-button-space div.stButton > button {
...
.back-button-space div.stButton > button:hover {
    background: #2563eb;
    color: white;
    border-color: #60a5fa;
...
def linear_interpolation(x_vals, y_vals, target_x):
    """
    Calculates Linear Interpolation using the two known points
    surrounding the target x-value and returns all intermediate
    values needed for a step-by-step explanation.
    """
    n = len(x_vals)

    if n < 2:
        raise ValueError("At least 2 data points are required for Linear Interpolation.")

    # Find the two points that surround the target value.
    # If the target is outside the data range, use the nearest
    # two end points and show an extrapolation warning in the UI.
    if target_x <= x_vals[0]:
        ...
        i = 0
        for j in range(n - 1):
            if x_vals[j] <= target_x <= x_vals[j + 1]:
                i = j
                break
    ...
    if np.isclose(delta_x, 0):
        raise ValueError("Linear Interpolation cannot be performed because x₀ and x₁ are equal.")

    slope = delta_y / delta_x
    x_difference = target_x - x0
    interpolation_part = slope * x_difference
    final_result = y0 + interpolation_part
...
back_left, back_space, back_right = st.columns([1.6, 5.8, 1.6])

with back_right:
    st.markdown('<div class="back-button-space">', unsafe_allow_html=True)
...
if calculate_button:
    ...
    if not is_valid:
        ...
        if selected_method == "Linear Interpolation":
            st.subheader("📐 Step-by-Step Calculation — Linear Interpolation")

            try:
                result = linear_interpolation(x_data, y_data, target_x)
                ...
                slope = result["slope"]
                x_difference = result["x_difference"]
                interpolation_part = result["interpolation_part"]
                final_result = result["final_result"]

                # STEP 1 — Formula
                st.markdown("### 🧮 STEP 1 — Use the Linear Interpolation Formula")
                st.latex(r"y = y_0 + \frac{y_1-y_0}{x_1-x_0}(x-x_0)")
                st.markdown(
                    """
                    <div class="step-box">
                        <b>Explanation:</b><br>
                        The formula connects two known points with a straight
                        line and estimates the value at the requested target x.
                        The fraction represents the slope (rate of change)
                ...
                # STEP 2 — Known values
                st.markdown("### 📌 STEP 2 — Plug in the Known Values")
                st.markdown(
                    f"""
                    <div class="step-box">
                        <b>First Point:</b> (x₀, y₀) = ({x0:.4f}, {y0:.4f})<br><br>
                        <b>Second Point:</b> (x₁, y₁) = ({x1:.4f}, {y1:.4f})<br><br>
                        <b>Target x:</b> x = {target_x:.4f}
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
                st.latex(
                    rf"y = {y0:.4f} + \frac{{{y1:.4f}-{y0:.4f}}}{{{x1:.4f}-{x0:.4f}}}({target_x:.4f}-{x0:.4f})"
                )

                # STEP 3 — Detailed calculations
                st.markdown("### 🔢 STEP 3 — Solve the Equation")

                # 1. Difference in y
                st.markdown("#### 1️⃣ Calculate the difference in y")
                st.latex(
                    rf"y_1-y_0 = {y1:.4f}-{y0:.4f} = {delta_y:.4f}"
                )
                st.success(f"Difference in y = {delta_y:.4f}")

                # 2. Difference in x
                st.markdown("#### 2️⃣ Calculate the difference in x")
                st.latex(
                    rf"x_1-x_0 = {x1:.4f}-{x0:.4f} = {delta_x:.4f}"
                )
                st.success(f"Difference in x = {delta_x:.4f}")

                # 3. Slope
                st.markdown("#### 3️⃣ Find the slope")
                st.latex(
                    r"\text{Slope} = \frac{y_1-y_0}{x_1-x_0}"
                )
                st.latex(
                    rf"= \frac{{{delta_y:.4f}}}{{{delta_x:.4f}}} = {slope:.6f}"
                )
                st.success(f"Slope = {slope:.6f}", icon="📈")

                # 4. Target distance
                st.markdown("#### 4️⃣ Calculate the target distance")
                st.latex(
                    rf"x-x_0 = {target_x:.4f}-{x0:.4f} = {x_difference:.4f}"
                )
                st.success(f"Target distance = {x_difference:.4f}")

                # Step 3 summary
                st.markdown("#### 📋 Step 3 Summary")
                s1, s2, s3 = st.columns(3)
                with s1:
                    st.metric("Δy", f"{delta_y:.4f}")
                with s2:
                    st.metric("Δx", f"{delta_x:.4f}")
                with s3:
                    st.metric("Slope", f"{slope:.6f}")

                # STEP 4 — Substitute and simplify
                st.markdown("### ✏️ STEP 4 — Substitute and Simplify")
                st.latex(rf"y = {y0:.4f} + ({slope:.6f})({x_difference:.4f})")
                st.latex(rf"y = {y0:.4f} + {interpolation_part:.6f}")
                st.latex(rf"y = {final_result:.6f}")

                # STEP 5 — Final answer
                st.markdown("### 🎯 STEP 5 — Final Answer")
                st.success(
                    f"Estimated value at x = {target_x:.4f}:  **f({target_x:.4f}) ≈ {final_result:.6f}**",
                    icon="✅",
                )
                ...
                with c1:
                    st.metric("Point 1", f"({x0:.2f}, {y0:.2f})")
                with c2:
                    st.metric("Point 2", f"({x1:.2f}, {y1:.2f})")
                with c3:
                    st.metric("Interpolated Value", f"{final_result:.6f}")

            except Exception as e:
                st.error(f"Unable to perform Linear Interpolation: {e}", icon="🚨")

        # EXECUTE NEWTON FORWARD
        elif selected_method == "Newton Forward":
